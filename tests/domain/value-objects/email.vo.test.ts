...
describe("Email value object", () => {
  it("Must throw InvalidEmailError when email is empty", () => {
    expect(() => new Email("")).toThrow(InvalidEmailError);
  });

  it("Must throw InvalidEmailError when email has invalid format", () => {
  ...
  it("Must throw InvalidEmailError when email length exceeds 255 characters", () => {
    const longEmail = "a".repeat(250) + "@test.com";
    expect(() => new Email(longEmail)).toThrow(InvalidEmailError);
  });

  it("Must clean spaces and lowercase the email", () => {
  ...
  it("validate method must return cleaned email", () => {
