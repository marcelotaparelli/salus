...
describe("Cpf value objects", () => {
  it("Must return InvalidCpfError when length is different from 11", () => {
    expect(() => new Cpf("213.304.555-0")).toThrow(InvalidCpfError);
  });

  it("Must return InvalidCpfError when all digits are equal", () => {
    expect(() => new Cpf("111.111.111-11")).toThrow(InvalidCpfError);
  });

  it("Must return InvalidCpfError when last digits are invalid", () => {
    expect(() => new Cpf("243.451.522-44")).toThrow(InvalidCpfError);
  });

  it("Methods must return a cleaned or formatted cpf", () => {
  ...
  it("isValid must return true for a valid CPF", () => {
    expect(Cpf.isValid("16703846081")).toBe(true);
  });

  it("Must clean non-digit characters", () => {
    const cpf = new Cpf("167.038.460-81");
